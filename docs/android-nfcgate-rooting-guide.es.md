# Guía — Rootear un teléfono Android para NFCGate (Path B, modo card)

> **Read this in another language:** [English](android-nfcgate-rooting-guide.en.md)

> **Manual complementario de [`relay` → Path B](commands/relay.md#path-b--against-the-nfcgate-android-app)
> y de la [Guía de Uso de BomberCat](https://github.com/ElectronicCats/BomberCat/blob/main/docs/guia-usuario-bombercat.es.md#modo-2--bombercat-reader--móvil-con-nfcgate-en-modo-card) (Modo 2).**
> Solo necesitas este documento si vas a usar el teléfono en **modo
> *tarjeta*/HCE** dentro de NFCGate (BomberCat como `reader`, teléfono
> emulando la tarjeta), que **exige un teléfono rooteado**. Si tu teléfono no
> está rooteado y no quieres rootearlo, usa el teléfono como **`reader`** en su
> lugar: hace lo mismo con los roles invertidos y **no requiere root**.
...
5. **Verifica en NFCGate.**
   ...
   avisos en rojo). Si el status está en verde, NFCGate está listo para usarse
   en **modo tarjeta/HCE**.

---

> **Vuelve al workflow:** una vez el teléfono está rooteado y NFCGate muestra su
> *status* correcto, continúa con
> [`relay` → Path B](commands/relay.md#path-b--against-the-nfcgate-android-app)
> (o con el [Modo 2 de la Guía de Uso de BomberCat](https://github.com/ElectronicCats/BomberCat/blob/main/docs/guia-usuario-bombercat.es.md#modo-2--bombercat-reader--móvil-con-nfcgate-en-modo-card)
> si llegaste desde ahí).
