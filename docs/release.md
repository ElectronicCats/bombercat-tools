...
3. **Commit and tag.**
   ...
   ```sh
   git add VERSION
   git commit -m "chore(release): v1.3.1.1"
   git tag v1.3.1.1
   git push origin main --tags
   ```

   `VERSION`'s 4-component form (`1.2.0.1`) and the git tag's semver-ish form
   (`v1.3.1.1`) are two different strings by convention — don't try to make
...
4. **Create the GitHub Release.** This is what triggers the four
   ...
   ```sh
   gh release create v1.3.1.1 --generate-notes
   ```
...
6. **Re-run a single failed build**, if one of the five didn't make it,
   ...
   ```sh
   gh workflow run build-mac.yml -f release_tag=v1.3.1.1
   ```
